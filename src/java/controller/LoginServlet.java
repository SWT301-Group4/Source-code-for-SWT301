...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc != null) {
            /**
             * save session to view home page
             */
            ...
            /**
             * save cookies if remember is selected
             */
            if ("on".equals(remember)) {
                Cookie uName = new Cookie("uName", username);
                Cookie uPass = new Cookie("uPass", password);
                Cookie remem = new Cookie("remem", remember);
                uName.setMaxAge(60 * 60 * 24);
                uPass.setMaxAge(60 * 60 * 24);
                remem.setMaxAge(60 * 60 * 24);
                response.addCookie(uName);
                response.addCookie(uPass);
                response.addCookie(remem);
            } else {
                Cookie uName = new Cookie("uName", "");
                Cookie uPass = new Cookie("uPass", "");
                Cookie remem = new Cookie("remem", "");
                uName.setMaxAge(0);
                uPass.setMaxAge(0);
                remem.setMaxAge(0);
                response.addCookie(uName);
